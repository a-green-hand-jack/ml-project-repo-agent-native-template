...
python scripts/adopt-existing-repo.py <repo> --phase all --origin <owner/repo>  # 迁移已有 repo 到 template 形态
...
`template-sync.py` 是分阶段事务（preflight/plan/apply/verify/commit-version）：`.template.toml` 的
version **只在** 文件 apply、Codex 生成器重建、`validate-governance.py` 全部成功后，才用「同目录临时
文件 + 原子替换」推进；任一步失败或超时都保持旧版本、进程非零，绝不出现「同步失败但版本已声称追平」
的错误事实。无论成败都写一份 sync receipt（默认 `.template-sync-receipt.json`，可 `--receipt PATH`
覆盖）：`result` 区分 `pass`/`partial`/`fail`/`unknown`（超时/中断记 `unknown`，永不显示 `pass`），
并绑定 exact 上游 SHA/digest、from/to version、expected/actual path manifest、逐阶段状态与失败阶段/
...
所有 phase 都要求显式 `--origin <owner/repo>`（不读 remote 猜测）。只有无 blocker 的成功
`normalize` 才在所有 move 后原子创建/确认 `.template.toml`；dry-run、scaffold-only 与
`--allow-blocked-normalize` 不写锚点。同 origin 的已有锚点保留 version；异源、malformed 或
symlink 锚点均 fail-closed，且会在 move 前预检。
...
`_template_anchor.py` 同样不是独立脚本：它是 bootstrap 与 adoption 共用的窄
`.template.toml` create/confirm/conflict helper，不参与 `template-sync.py` 的 version-advance 事务。
