...
| `check-doc-lifecycle.py` | brief/plan/review/decision 生命周期：唯一顶部状态锚点↔注册表一致、引用完整、活跃 plan 的 issue/Git branch/worktree 关联（含 detached exact/synthetic-merge checkout）、进阶态证据、过期 approval；`pretooluse_reason()` 供 pre_tool_guard 机械拦截复用（含 anchored apply_patch 重建、Bash wrapper 归一化、GNU cp option abbreviation/PWD expansion）；`--self-test` 跑内嵌对抗 fixtures | `plans/ANATOMY.md` · `plans/20260712-plan-lifecycle-state.zh.md` |
