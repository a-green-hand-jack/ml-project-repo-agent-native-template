...
- 两类脚本，别混着看：**只读校验脚本**（`check-agent-harness.py` / `check-anatomy-drift.py` /
  ...
  `sync-codex-adapters.py` / `bump-template-version.py` / `template-sync.py` / `init-governance-data.py`）
  会写文件/跑 `git config` 等，改动前确认目标 repo 路径正确（`init-governance-data.py` 默认写、
  `--dry-run` 才只读预览）。全部无第三方硬依赖。
