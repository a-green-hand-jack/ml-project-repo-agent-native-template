...
RUN_STATUS = {"planned", "approved", "running", "done", "failed", "superseded"}
...
def _self_test() -> int:
    ...
    # 正例：#16 状态机中的 approved run 合法存在，但未被证据引用时不算闭环来源。
    def approved_unreferenced(root: Path, digest: str) -> None:
        _write(
            root,
            "lab/research/experiment-ledger.yaml",
            _GOOD_LEDGER
            + "  - id: run-002\n"
              "    question: approved control-plane run\n"
              "    status: approved\n"
              "    commit: abc1234\n"
              "    config: lab/code/configs/exp2.yaml\n"
              "    data_split: none\n"
              "    expected_runtime: under 1 minute\n"
              "    success_metric: control plane passes\n",
        )
    _run_case("positive-approved-unreferenced", approved_unreferenced, [], True, failures)
