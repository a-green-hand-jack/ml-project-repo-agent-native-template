...
- 首轮**自命名**：无 `--title` 时按 doctrine 生成名 → `paseo rename "$PASEO_AGENT_ID" <name>` + 写 identity + 注册 `memory/agents-roster.md`。
...
## 任务树（供批注确认拆分）
- [ ] Phase 1：doctrine + statusline `🤖` 段 + identity 文件
- [ ] Phase 2：自命名(`paseo rename`) + `memory/agents-roster.md` + 自知 hook
- [ ] Phase 3：`spawn` **skill**（发现 + 选型/packet + 两层交代 + in-session/Paseo-tab launcher）
