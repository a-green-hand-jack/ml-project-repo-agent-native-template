...
- `.codex/rules/default.rules`（若新增可独立手动运行的 provenance 脚本，为其补精确的
...
- [ ] 6. 正负向 fixture + CI 集成
  ...
  - [ ] 6.5 若新增独立 `scripts/check-provenance-chain.py`，同步在
...
- [ ] 新 provenance 校验确认由 `validate-governance.py` 拉起（CI 与手动同源），触发与 runtime
      无关；实现完成后在 Codex 侧实跑并确认输出包含新增子检查（现有 validator 的 Codex 手动
      执行基线已于本轮通过）。
