...
- [ ] 2. 引用链校验器（run → artifact → evidence → claim → deliverable）
  - [ ] 2.0 校验器实现落在新建独立脚本 `scripts/check-provenance-chain.py`（已决策，仿
        ...
        `run_subcheck` 拉起；不写进 `validate-governance.py` 现有函数体内
...
- [ ] 3. checksum / manifest 支持
  ...
  - [ ] 3.5 checksum 计算走 `hashlib` **在进程内完成**，不 shell-out 到 `sha256sum`/`md5sum`：
        ...
        runtime 间的 Bash allowlist 差异；进程内 hash 也减少外部命令与平台差异。
...
- [ ] 6. 正负向 fixture + CI 集成
  ...
  - [ ] 6.5 新建独立脚本 `scripts/check-provenance-chain.py` 后，同步在
...
- [ ] 7. 文档与治理面同步（same-commit）
  ...
  - [ ] 7.6 双 runtime 触发对等 smoke（能做多少做多少）：至少确认新 provenance 校验作为
        ...
        拉起，输出与调用它的 runtime 无关；实现完成后在 Codex 侧再手动跑同一命令，核对新增
        provenance 子检查确实出现在输出中。当前真实 Codex session 已证明现有 strict validator
        可直接运行并以 exit 0、0 error / 0 warning 结束，但尚无新增脚本可验证。
...
5. **已决策：新校验器为独立脚本**（human 拍板，2026-07-12，不再是 open question）——
   `scripts/check-provenance-chain.py`，仿 `scripts/check-same-commit.py` 先例：`scripts/`
   现有惯例是「可单独跑 + 无第三方硬依赖」，独立脚本便于单独测试、复用，且不无限膨胀
   `validate-governance.py` 单文件体积。硬约束（二审已定，本轮强化）：必须由
