...
Status: implementing · 2026-07-18 · ref: #88 执行阶段 kickoff（总调度授权重新 clone lillian039/ELF + case 分支 commit/push，不开 PR/merge/close）；ELF v2 冻结 adoption baseline 落成，证据 lab/docs/audits/elf-refresh-v2-baseline.md 与 memory/branches/88-elf-refresh-v2-A.md

> 这是 human 与 Claude Code 的协商界面：agent 写初稿 → human 在文件里批注 → agent 读 diff、
> 收敛计划 → 每次采纳的修订做一个小 commit。设计阶段（draft）只覆盖「执行前的刷新方案」的
> 设计与决策收敛。**（2026-07-18 更新）设计已定稿并进入执行 kickoff**：#88（更新者 A）完成
> 「重新 clone + 冻结 adoption baseline」这一子集，状态推进为 `implementing`；#84 targeted smoke
> 与 #85 full replay/mutation + ledger 收口分别承接后续阶段（一次只激活一个）。
...
- [x] #88 执行阶段 kickoff（更新者 A，2026-07-18）：`case/elf-refresh-v2` 分支 + 升 `implementing`
      + 重新 clone `lillian039/ELF`@`5098bf2`（gitignored external/）+ 落冻结 adoption baseline
      （provenance / classification / manifest / template anchor），证据
      `lab/docs/audits/elf-refresh-v2-baseline.md` + `memory/branches/88-elf-refresh-v2-A.md`
- [ ] #84 targeted smoke（测试者 B，从冻结 handoff SHA 独立复验）
- [ ] #85 full replay / mutation matrix + ledger v2 收口（追加 v2 条目 + 旧条目标 superseded）
...
- 2026-07-18 执行 kickoff #88（干将·迁·ELF基线，更新者 A）：draft→`implementing`。总调度授权重新
  clone `lillian039/ELF`@`5098bf2` 落 gitignored `lab/code/external/ELF/`；产出冻结 adoption
  baseline——provenance（`lab/docs/reference/provenance.md`）、ownership/classification（adopt
  discover dry-run：root_entries=5、protected=0、conflict=1[README]、conservative_import=4）、
  migration manifest + 等价 template anchor（VERSION v1.3.8 @ base `7c09e90`），证据
  `lab/docs/audits/elf-refresh-v2-baseline.md`。doc-lifecycle 补 branch=`case/elf-refresh-v2` /
  worktree / approval。**未做**（留 #84/#85）：探针、full replay、ledger v2、旧条目 superseded、
  模板 bug 修复。不开 PR/merge/close，验收留主调度。
