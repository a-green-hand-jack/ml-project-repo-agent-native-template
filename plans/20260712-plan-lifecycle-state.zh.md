...
- 2026-07-12 **进入实现（status → implementing，agent 自主标记，据已决策 10）**。落地：`memory/doc-lifecycle.yaml` 注册表（回填 4 plan + 3 decision）；`scripts/check-doc-lifecycle.py`（judgment 本体 + 内嵌 fixtures self-test，PyYAML 可选/受限解析器兜底）；`validate-governance.py` 经 run_subcheck 拉起；`pre_tool_guard.py` 薄接线 doc-lifecycle 拦截（`DOC_LIFECYCLE_SKIP=1` 逃生，判定层异常保守放行）；模板/skill/doctrine/入口纪律/ANATOMY 接线与存量 Status 锚点回填。设计细化（在 plan 定案框架内）：scope/forbidden/verification 必填与批注收敛检查作用于 `approved`/`implementing`（verified/superseded 为历史态不追溯——否则存量回填要求重写旧 plan 正文，违反"只加一行"）；apply_patch Update 无法重建全文，hook 用"当前文件+新增行"联合语料只可能漏拦不误拦，validator 事后兜底。**已决策 11(b) 的 startup 扩展评估结论：本轮不扩展**——入口纪律（CLAUDE.md/AGENTS.md 先读第 3 条 + current-status.md 当前 plan 指针）已确定性覆盖 fresh startup；给 SessionStart(startup|resume) 挂 continuity 会让每个 fresh session 固定吃 ~4KB 注入，与 context-orchestration"只在边界注入"原则冲突；若实践中仍漏读指针，后续只需两侧各改一行 matcher 即可扩展（记录于 plans/ANATOMY.md）。
- 2026-07-13 **折叠 Codex 初审（gpt-5.6-sol high）4 条 MAJOR**。(1) apply_patch Update 绕过：废弃「旧全文+新增行」联合语料（旧全文在前会先读到旧状态，静默放行 verified→approved 跃迁），改为按 hunk 重建 patch 后全文再判定（与 Edit 同语义），注册表 Update 同样重建判定；无法可靠重建时**保守拦截**并提示改用 Edit——宁可要求换工具，不静默放行。(2) 删除注册表关闭治理：hook 拦 apply_patch Delete/Move 与 Bash rm/mv/git rm 触碰 `memory/doc-lifecycle.yaml`；validator 侧「存在受管文档但注册表缺失」由 warning 升为 error（非 strict 也 fail）。(3) kind 谎报绕过必填段校验：注册表 kind 必须与路径类别一致（plans/ ⇒ plan、human/decisions/ ⇒ decision 等），不一致即 error（hook+validator 双侧）；路径不在四类目录才允许自由声明 kind（告警）。(4) 冒烟证据落 diff：新建 `lab/evals/doc-lifecycle/`——`run-continuity-probes.py` 可复跑 synthetic 探针（startup 0 输出 / clear/PostCompact 回注含当前 plan 指针，实跑记录 `evidence-20260713-synthetic-probes.md`）+ `runtime-smoke-checklist.md`（真实 fresh Claude/Codex session 的待跑清单与证据模板，**留给监控员**，verified 前必须补齐）。4 个 PoC 全部做成内嵌负向 fixtures（self-test 15→30 例），pre_tool_guard 安全地板 synthetic 回归零弱化。
- 2026-07-13 **折叠 fresh review（HEAD c5e925d）3 条代码问题，真实 runtime 门槛不冒充完成**。(1) `apply_patch Update` 重建保留并唯一定位 `@@ <anchor>`；重复/缺失 anchor 无法证明落点时保守拦截，回归覆盖两个相同片段但只允许 Human 批注区变化的绕过。(2) Bash 删除判定展开 `command`/`env` wrapper，并跳过 `git -C`、`--literal-pathspecs` 等全局选项；四个原绕过均进入判定层+self hook regression。(3) 活跃 plan（approved/implementing）要求非占位规范 issue 坐标与现存 Git branch，implementing worktree 必须是 `git worktree list` 中绑定该 branch 的真实条目；verified 历史态允许合并后清理临时 branch/worktree，由 approval 的 commit/PR/test 证据追溯。真实 Claude/Codex fresh/clear/compact 与 hook trust 清单仍未跑，保持 implementing，列为 merge blocker。
- 2026-07-13 **折叠 exact-head fresh review（HEAD d96c71d）2 条 MAJOR**。受限 YAML parser 现在拒绝 `docs: []` 后继续缩进条目的歧义结构，避免无 PyYAML 时隐藏条目并让 hook fail-open；active plan 使用 `worktree: .` 按声明 branch 从 `git worktree list` 自动发现真实 checkout，使 linked worktree、独立 clone 与主 checkout 验证一致，显式路径仍严格精确匹配。
- 2026-07-13 **折叠 exact-head fresh review（HEAD 11cf28a）1 条 MAJOR**。受限 YAML parser 现在拒绝重复顶层 `docs` 字段，避免追加 `docs: []` 时 fallback 保留旧条目而 PyYAML 采用末值的语义分叉；self-test 与真实 `python -S` hook regression 都锁定 fail-closed。
- 2026-07-13 **真实隔离 Codex 顶层探针发现并修复 stdout 协议 blocker**。exact HEAD `02626c3` 的 `/clear`、`/compact` 真实 TUI 分别报 `invalid session start JSON output`、`invalid user prompt submit JSON output`、`invalid PostCompact hook JSON output`；此前 synthetic 只搜裸文本，未按宿主 schema 解析，因而漏检。按 Codex 0.144 官方源码 schema 修复三个注入 hook：非空 stdout 统一为单个 `hookSpecificOutput.additionalContext` JSON；continuity 从不支持模型注入的 `PostCompact` 改挂明确支持 `source=compact` 的 `SessionStart(compact|clear)`。探针扩展为 8 个协议/静默对抗 case，全部通过；真实 C/X/G 清单仍须在该修复 commit 上重跑，失败记录不转 PASS。
- 2026-07-13 **真实 runtime 验收 8/8 PASS（目标代码 `68f1d43`）**。隔离 Claude 顶层 session 完成 C1 fresh、C2 clear、C3 compact；C3 首次因消息不足未计 PASS，在同一隔离 session 累积只读多轮后第二次真实 compact 成功并捕获 `SessionStart:compact success` + continuity。隔离 Codex 顶层 session 完成 X1 fresh、X2 clear、X3 compact，无 invalid hook JSON。disposable clone 中真实 Claude tool call 完成 G1 默认拒绝 + 独立 `DOC_LIFECYCLE_SKIP=1` 放行，以及 G2 注册表删除拒绝。raw 转录/debug log、session id 与 SHA256 落 `lab/evals/doc-lifecycle/`；状态保持 implementing，等待 evidence-only exact-head fresh review、合入与合并后验证。
- 2026-07-13 **折叠 evidence-only HEAD `e3facb2` 的 fresh Codex CHANGES_REQUESTED 与四轮代码复审**。修复并以对抗 fixtures 锁定：空/缺失 registry、完整 Status anchor、approval/required-section placeholder、PyYAML fallback scalar parity；Bash wrapper/cwd/newline/glob/pathspec/ancestor/symlink/overwrite 绕过；受管 plan 的 symlink canonical identity；引号/转义 glob 与根目录 `*` 的安全 control。修复提交 `06c98f2` 在最终独立 Codex review 获 `VERDICT: APPROVE`；self-test、真实 hook regression、continuity、adapter sync、strict harness/anatomy/governance 全绿。
- 2026-07-13 **guard runtime G1/G2 在精确代码 `06c98f2` 重跑 PASS**。两个独立 Claude 顶层进程完成 G1 默认拒绝与仅设置 `DOC_LIFECYCLE_SKIP=1` 的放行对照；第三个顶层进程执行 `env -C memory rm doc-lifecycle.yaml`，由 PreToolUse 拒绝且独立 disposable clone 的注册表保持存在。C1-C3/X1-X3 所覆盖的 continuity/context hook 从 `68f1d43` 至今未变，保留原真实 evidence；新增 G raw/debug log 与 SHA256 已落盘。状态仍为 implementing，等待 evidence-only exact-head fresh review。
- 2026-07-13 **最终代码 parent `492e042` 的 G1/G2 再次真实重跑 PASS**。在合入本地 main 已完成的 #12/#14/#15/#16/#17 语义并集后，受限 parser 又按 exact-head review 修复 inline `docs` 子项、portable worktree 绑定、重复顶层 `docs` fail-closed，并以真实 `python -S` hook 对抗 fixture 锁定。三个新 disposable clone 中，默认非法 Write deny、仅显式 `DOC_LIFECYCLE_SKIP=1` allow、registry delete deny 均由独立 Claude 顶层 session 完成；raw 转录/debug log 与 SHA256 绑定本代码 parent。状态保持 implementing，等待 evidence-only exact HEAD 独立最终 review。
- 2026-07-13 **折叠 evidence HEAD `54822ff` fresh review 的 PyYAML duplicate-root MAJOR**。reviewer 用第二份本身合法的 `docs:` 复现：正常 PyYAML 路径 `errors=[]` 且 hook 放行，`python -S` fallback 则拒绝；旧 fixture 追加 `docs: []` 只是被空注册表规则间接拦截。修复在 PyYAML AST 构造阶段统一拒绝重复顶层 `docs`，self-test 与 normal/`python -S` 真实 hook regression 都改用第二份合法列表，消除双 parser 语义分叉。状态继续 implementing，等待新 exact code target 的 G1/G2 raw evidence、严格门禁与独立 final review。
- 2026-07-13 **最终代码 parent `de9aa2e` 的 G1/G2 exact-target runtime 再验 PASS**。三个全新 disposable clones 中，独立 Claude Code 2.1.207 顶层 session 分别完成默认非法 plan Write deny、仅显式 `DOC_LIFECYCLE_SKIP=1` allow、`env -C memory rm doc-lifecycle.yaml` deny；reject/delete clones 均 clean 且注册表仍存在，skip 产物只留在临时 clone。六份 typescript/debug 原文以 `gzip -n` 与 SHA256 落盘，状态仍为 implementing，等待 evidence-only direct child 的 exact HEAD 独立 final review。
- 2026-07-13 **折叠 evidence HEAD `d57957c` final review 的三个 MAJOR**。(1) branch 绑定改为本地 branch 优先、detached clone 回退到唯一同名 remote commit；`worktree: .` 只接受当前 detached exact tip 或以该 tip 为直接父提交的双亲 synthetic merge，普通线性后继与 remote 分叉 fail-closed，内嵌 fixture 真实创建 source/clone/remote refs/commit graph 验证。(2) 删除 `memory/current-status.md` 中“#13 仍缺 runtime smoke”的旧结论，与顶部 8/8 PASS 单一真相对齐。(3) 状态锚点现在必须是标题后的第一条非空正文且全文唯一，代码围栏示例不计；hook 对缺失/重复/非顶部/fenced-only 全部 fail-closed，normal 与 `python -S` synthetic 回归同步覆盖。状态继续 implementing，须在新 code target 重跑 G1/G2、strict gates 与独立 exact-head final review。
- 2026-07-13 **最终代码 parent `6950825` 的 G1/G2 exact-target runtime PASS**。三个新 disposable clones 与独立 Claude Code 2.1.207 / Opus 4.8 顶层 sessions 分别证明：默认 invalid plan Write 被 `doc-lifecycle:` deny 且 clone clean；仅单进程 `DOC_LIFECYCLE_SKIP=1` 后同一 Write 成功；`env -C memory rm doc-lifecycle.yaml` 被 deny、clone clean 且 registry 存在。六份 typescript/debug 原文以 `gzip -n` 与 SHA256 落盘；下一门槛为 evidence-only direct child 的全 strict gates 与独立 exact-head final review。
- 2026-07-13 **evidence HEAD `8b4c55b` 独立 final review 的五个 MAJOR**。(1) `git rm --pathspec-from-file` 的文件/stdin/NUL 输入无法由 hook 核验，破坏注册表的命令一律 fail-closed；(2) 状态锚点只认普通正文，blockquoted-only 必须缺锚点，真实锚点加 blockquoted 示例不得误判重复；(3) detached synthetic merge 只接受恰好双亲，octopus 即使含 feature tip 也拒绝；(4) `issue`/`branch`/`worktree`/`approval` 在所有状态都必须是 string 或 null，draft 非标量在 PyYAML/受限 parser 与 normal/`python -S` hook 均 fail-closed；(5) durable status 改为指向本次 review，而非已完成的 evidence 提交步骤。Null/NULL/~ fallback parity 与非标量对抗矩阵继续保留。状态仍 implementing；修复提交后必须在精确新 target 重跑 G1/G2、全 strict gates 与独立 final review。
- 2026-07-13 **最终代码 parent `b7c7c7a` 的 G1/G2 exact-target runtime PASS**。G1 在两个全新 disposable clones 中用同一中性 malformed-draft fixture，独立 Claude 顶层 sessions 证明默认 Write 被 `doc-lifecycle:` deny 且 clone clean、仅单进程 `DOC_LIFECYCLE_SKIP=1` 后 Write 成功且产物只留临时 clone。首次以 approved 载荷运行的 skip attempt 在 tool call 前被模型拒绝，原文保留但不计 PASS；未因单次不足停下。G2 使用真实指向 `memory/doc-lifecycle.yaml` 的 opaque `git rm --pathspec-from-file=<file>`，第三个顶层 session 被 hook deny、clone clean、registry 存在。八份 raw/debug 以 `gzip -n` 与 SHA256 落盘；下一门槛为 evidence-only direct child 的全 strict gates 与独立 exact-head final review。
