...
- [ ] **adoption dry-run 归类计划**：
      ```bash
      python scripts/adopt-existing-repo.py <fixture> --phase discover --dry-run --origin <owner/repo>
      ```
...
- [x] **真实 repo replay（tracked-byte integrity + smoke，新 repo，不复用 Agent-R1）**：`<real-repo>`
      ...
      ```bash
      python scripts/adopt-existing-repo.py <real-repo> --phase all --policy conservative --project-name <slug> --origin <owner/repo>
