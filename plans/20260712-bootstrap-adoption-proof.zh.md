...
- [ ] Parent：issue #12 三类生命周期增强
  ...
  - [ ] D. 文档/结构同步（`feat/12-bootstrap-adoption-proof` 落地归属 A 的子项：D1/D2 的 bootstrap
    ...
    - [x] D6：`python scripts/validate-governance.py --strict` + `check-agent-harness.py --strict` +
          ...
          全绿；另跑 `lab/evals/adoption/run-adoption-smoke.py`（2026-07-13 修正后为五场景全过；
...
- [x] **负向 fixture 不静默通过**：新增/扩展 `lab/evals/adoption/run-adoption-smoke.py`（或新脚本），
      ...
      **实现（2026-07-12；2026-07-13 按 Codex 初审 2 条 MAJOR 修正）**：
      `lab/evals/adoption/run-adoption-smoke.py` 现为五场景（happy-path 之外四个负向）：
      ...
      - `scenario_smoke_undetected`（无可探测命令，断言 `skipped` + 同样的 exit-0-with-warning
        组合）。
      `python3 lab/evals/adoption/run-adoption-smoke.py` 五场景全过（2026-07-13 真实重跑，
      Python 3.12.3）；此前「四场景全过」的证据声明不成立，以本条为准。
- [x] **文档/结构同步**（针对本分支 A 的改动集验证；B/C 落地后各自分支还需再跑一遍）：
      ...
      **C 补充（2026-07-12，`feat/12c-smoke-contract`）**：对应 C 的改动集重跑同一组命令 +
      ...
      另跑 `lab/evals/adoption/run-adoption-smoke.py`（2026-07-13 修正后为五场景全过；初版
...
- 2026-07-12 实现（Claude，`feat/12c-smoke-contract` worktree，base 是 `feat/12-bootstrap-adoption-proof`）：
  ...
  `lab/evals/adoption/run-adoption-smoke.py` 从单一 happy-path 扩成五场景（happy-path 回归 +
  `scenario_blocked_normalize` + `scenario_blocked_conflict` + `scenario_smoke_failing_command` +
  `scenario_smoke_undetected`），冲突文件与受保护路径各自独立 fixture。**2026-07-13 按 Codex 初审
  ...
  `lab/evals/adoption/run-adoption-smoke.py`（五场景，2026-07-13 修正后重跑）、
