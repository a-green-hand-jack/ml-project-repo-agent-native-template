...
- [ ] Parent：issue #12 三类生命周期增强
  - [ ] A. 新项目 bootstrap 命令
    ...
    - [ ] A4：Codex/Claude 项目配置加载清单（bootstrap 完成后打印/写入）。这条不是「抄一遍验收标准」，
          ...
          - 清单的**正确性不能靠手写维护**：以 `check-agent-harness.py`（检查项 #5 配置可解析、#6 Codex
            adapters 同步）为机器事实源，清单文案与该 validator 的判定保持一致，避免清单与真实加载漂移。
    - [ ] A5：synthetic fixture：在空 Git repo 上跑两次，断言第二次幂等 + 在**该 bootstrapped repo 内**
          ...
          三者全绿——即证明 bootstrapped repo 对 Claude 与 Codex **两侧**都是自洽可加载的（静态双 runtime
          冒烟；我们无法在此环境真正驱动 Codex 交互式会话，故以「adapter 齐全 + harness 绿」作为可得的对等证据，
          能否/是否需要真跑一次 Codex 会话见「未解决问题」）。
...
- **双 runtime 对等是本 feature 的一等公民，不是事后补丁**：任何 canonical（`.claude/**`）能力改动都要经
  `sync-codex-adapters.py` 生成 Codex 侧并 same-commit 提交，bootstrap 的验收把「Codex adapters 齐全 + harness
  绿」纳入 fixture。由于本环境无法真正驱动 Codex 交互式会话，Codex 侧「能否等价执行」以静态对等证据表态，
  真跑一次 Codex 会话与否留给 human 决定（未解决问题 8）。
...
8. **【Codex 二审新增】双 runtime 验证深度**：我们在本环境无法真正驱动一次 Codex 交互式会话来端到端验证
   bootstrapped/adopted repo 在 Codex 下的行为。A5/D6 采用的是「静态对等证据」——`.codex/agents/*.toml` +
   `.agents/skills/*` 齐全、`sync-codex-adapters.py --check` 与 `check-agent-harness.py --strict` 全绿。
   这够不够作为「Codex 侧可等价执行」的验收？还是要求 human 在真实 Codex 里手工跑一次 bootstrap/adoption
   做一次性人肉冒烟并回填报告？（我倾向前者足够，但这是无一手 Codex 运行证据下的判断，请 human 确认。）
9. **【Codex 二审新增】Codex trust 前提如何在验收里表态**：Codex 的 `config.toml` hooks 需 human 先 trust
   本 repo 才生效（`.codex/config.toml:1`），这一步无法脚本化。bootstrap 的「加载清单」把它列成 blocker/待办
   （类比 CODEOWNERS）是否是可接受的表态方式？还是验收标准 2 期望 bootstrap 输出里对 Codex 侧只列「文件已就位」
   而把 trust 明确标为「out-of-band、需 human 手动」？
10. **【Codex 二审新增】adoption（B/C）是否也要处理 Codex 落地**：`adopt-existing-repo.py` 当前聚焦 tracked-byte
    integrity + 结构归一，不负责在被迁移 repo 里 `git config core.hooksPath` / 生成 Codex adapters / 提示 trust。
    验收标准 2 的「加载清单」是只挂在 bootstrap（空 repo）场景，还是 adoption 完成后也应产出一份等价的
    Codex/Claude 加载清单？（若是后者，B/D 需要各加一条任务；本 plan 目前只在 A4 覆盖了 bootstrap 场景。）

## 验证标准

对应 issue 验收标准，逐条给出要跑的命令/fixture：

- [ ] **bootstrap 幂等 + 双 runtime 静态自洽**：在空 Git repo 上跑两次 `scripts/bootstrap-project.py`，
