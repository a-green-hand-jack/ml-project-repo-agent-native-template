...
- [ ] Parent：issue #12 三类生命周期增强
  - [x] A. 新项目 bootstrap 命令（实现见 `scripts/bootstrap-project.py` + `.claude/skills/bootstrap-project/SKILL.md`，
        2026-07-12，`feat/12-bootstrap-adoption-proof`；A5 有一项已知遗留，见下）
    ...
    - [~] A5：synthetic fixture：在空 Git repo 上跑两次，断言第二次幂等 + 在**该 bootstrapped repo 内**
          ...
          **实现状态（2026-07-12）**：synthetic fixture 部分已完成并绿——`lab/evals/bootstrap/run-bootstrap-smoke.py`
          ...
          session 的 runtime smoke 部分未完成**：这需要一个真实 Codex CLI session 对着 bootstrapped repo 跑，
          执行本任务的 Claude subagent 没有这个 runtime 可用，只能如实记录为遗留（见 `memory/current-status.md`
          Open issues），留给 human 或后续 Codex session 补齐，不冒充已验证。
  ...
  - [ ] D. 文档/结构同步（本分支只落地归属 A 的子项：D1/D2 的 bootstrap 新增部分/D2b/D3/D4/D5/D6；
        D2 的 adopt-existing-repo 更新部分与 D2c 留给 B 的分支）
    ...
    - [x] D3：`README.md`、`DESIGN.md` §10 能力清单数量同步（新增 script/skill/command 时 Claude 侧与
          Codex 生成侧的计数都要对上）
    - [x] D4：`memory/current-status.md` / `session-tree.md` 记录本 feature 落地状态
...
- [~] **bootstrap 幂等 + 双 runtime 静态自洽 + Codex fresh-session smoke**：在空 Git repo 上跑两次
      ...
      ```
      三个静态检查已全绿（**静态自洽部分完成**）。**Codex fresh-session smoke 部分未完成**：需要从
      `<empty-repo>` 启动一个真实 fresh Codex session 保存 guidance/skill discovery 与 project hook
      加载的最小证据，这需要真实 Codex CLI runtime，执行本任务的 Claude subagent 不具备，如实记录为
      遗留（见 `memory/current-status.md` Open issues 与本文件 A5 状态记录），不冒充已验证。
...
- 2026-07-12 实现（Claude, `feat/12-bootstrap-adoption-proof` worktree）：落地任务树 A（A1-A6）+
  ...
  `git diff --check` 全绿。**已知遗留**：A5 的 fresh-Codex-session runtime smoke（guidance/skill
  discovery + project hook 加载 provenance）未执行——需要真实 Codex CLI session 对着 bootstrapped
  repo 跑，执行本次实现的 Claude subagent 不具备该 runtime，如实记录为待办而非冒充已验证，详见
  `memory/current-status.md` Open issues。B（语义归类）、C（smoke 合同）与 D2c 按既有决策/上层任务
  边界未在本分支涉及。未 push、未开 PR、未 merge——按边界交由上层/human 决定后续 gate。
