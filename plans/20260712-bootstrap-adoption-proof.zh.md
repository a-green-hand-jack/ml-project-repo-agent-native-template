...
- [ ] Parent：issue #12 三类生命周期增强
  ...
  - [ ] D. 文档/结构同步（`feat/12-bootstrap-adoption-proof` 落地归属 A 的子项：D1/D2 的 bootstrap
        新增部分/D2b/D3/D4/D5/D6；`feat/12c-smoke-contract` 在其之上补齐归属 C 的部分：D1（本轮未新增
        顶层路径，无需改动）、D3（README/`scripts/README.md` 的 smoke 合同措辞同步，本轮未新增
        script/skill/command，故 `DESIGN.md` §10 数量不变）、D4（`memory/current-status.md` /
        `session-tree.md` 记录 C 的落地状态）、D6（对应 C 改动集重跑全部 validator 全绿）；D2 的
        adopt-existing-repo `SKILL.md` 更新部分与 D2c 仍留给 B 的分支）
    - [x] D1：`scripts/ANATOMY.md`、`.claude/ANATOMY.md`、root `ANATOMY.md`（如涉及新顶层路径）——
          本轮未新增顶层路径，root `ANATOMY.md` 不需要改动
    - [~] D2：`.claude/skills/adopt-existing-repo/SKILL.md` 更新步骤说明（**未做，归属 B，见 B1-B4 语义
          归类改动**）；`bootstrap-project` skill 新增（**已做**：`.claude/skills/bootstrap-project/SKILL.md`）
    ...
    - [ ] D2c：bootstrap 与 adoption 共用同一份 agent-surface postflight 数据结构/渲染函数，避免两套
          Claude/Codex 加载清单发生文案和判定漂移。**明确排除在本分支范围外**（上层任务指令排除 D2c；
          `bootstrap-project.py` 已自带独立的 `agent_surface_checklist()`，留给 B 落地 B6 时决定是否重构复用）。
    ...
    - [x] D6：`python scripts/validate-governance.py --strict` + `check-agent-harness.py --strict` +
          ...
          全绿；另跑 `lab/evals/adoption/run-adoption-smoke.py`（四场景全过）与
...
- [x] **负向 fixture 不静默通过**：新增/扩展 `lab/evals/adoption/run-adoption-smoke.py`（或新脚本），
      ...
      **实现（2026-07-12）**：`lab/evals/adoption/run-adoption-smoke.py` 新增
      `scenario_blocked_normalize`（受保护路径 `checkpoints/`，断言两脚本均非 0 exit + blocker
      文本/`--json unresolved_blockers` 可读）、`scenario_smoke_failing_command`（检测到但跑失败，
      断言两脚本 exit 0 + report/`--json smoke_warnings` 非空）、`scenario_smoke_undetected`
      （无可探测命令，断言 `skipped` + 同样的 exit-0-with-warning 组合）。`python
      lab/evals/adoption/run-adoption-smoke.py` 四场景全过。
- [x] **文档/结构同步**（针对本分支 A 的改动集验证；B/C 落地后各自分支还需再跑一遍）：
      ...
      **C 补充（2026-07-12，`feat/12c-smoke-contract`）**：对应 C 的改动集重跑同一组命令 +
      ...
      另跑 `lab/evals/adoption/run-adoption-smoke.py`（四场景全过）与
      `lab/evals/bootstrap/run-bootstrap-smoke.py`（回归，确认未破坏 A）。
...
- 2026-07-12 实现（Claude，`feat/12c-smoke-contract` worktree，base 是 `feat/12-bootstrap-adoption-proof`）：
  ...
  `lab/evals/adoption/run-adoption-smoke.py` 从单一 happy-path 扩成四场景（happy-path 回归 +
  `scenario_blocked_normalize` + `scenario_smoke_failing_command` + `scenario_smoke_undetected`），
  覆盖验证标准要求的三类负向场景（合并成三个 fixture，一个覆盖冲突/受保护两个子类）。真实 repo
  ...
  `lab/evals/adoption/run-adoption-smoke.py`（四场景）、`lab/evals/bootstrap/run-bootstrap-smoke.py`
  （回归）全绿。B（语义归类）与 D2/D2c 未在本分支涉及。未 push、未开 PR、未 merge——按边界交由上层/
