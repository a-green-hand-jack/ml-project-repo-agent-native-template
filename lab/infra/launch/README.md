...
| `expctl.py` | 控制面 CLI：`detect`（adapter 可用性/降级）/ `plan`（命令草案，不执行）/ `watch`（一次性有界快照检查，只读）/ `apply-recovery --dry-run`（校验 ledger 提案；缺可信 provenance/原子 consumer，actual execution fail-closed） |
...
| `launch_gate.py` | 门禁判定模块：被共享 `pre_tool_guard.py` hook 加载；registered launch 命中永拒，env split/shell eval 动态面整体 fail-closed，caller-set env 不放行 |
