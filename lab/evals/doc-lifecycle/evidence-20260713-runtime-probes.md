...
> `b7c7c7a52a58f32a5ad5c2855016becf69baf944`。
> 所有 session 都在 `/tmp/issue-13-18-fresh.UBWQdD/` 或
> `/tmp/issue13-g12-b7c7c7a.6bhXKe/` 下的隔离 HOME 与 disposable clone 中启动；未修改真实
...
| G1 | Claude Code 2.1.207 · Opus 4.8 | reject `75074537-55e7-4649-b286-b6db5e5b88de`; skip `cb9e78d8-4ca9-47c7-a458-6cd976311bc2` | PASS（目标 `b7c7c7a`）：两个全新 clone 使用同一 malformed-draft fixture；默认 fresh session 的真实 Write 被 `doc-lifecycle:` 拒绝且 clone clean，独立 fresh session 仅设置 `DOC_LIFECYCLE_SKIP=1` 后同一 Write 成功且产物只在 disposable clone | `raw/13-G1-reject-claude-b7c7c7a.typescript.gz`, `raw/13-G1-reject-claude-debug-b7c7c7a.log.gz`, `raw/13-G1-skip-claude-b7c7c7a.typescript.gz`, `raw/13-G1-skip-claude-debug-b7c7c7a.log.gz` |
| G2 | Claude Code 2.1.207 · Opus 4.8 | `7433f5a1-013a-4dc0-a0b6-edf25978a751` | PASS（目标 `b7c7c7a`）：真实 Bash `git rm --pathspec-from-file=<opaque-file>` 载荷实际指向注册表，被 `doc-lifecycle: 禁止删除/移走` 拒绝；disposable clone clean、注册表仍存在 | `raw/13-G2-delete-claude-b7c7c7a.typescript.gz` + `raw/13-G2-delete-claude-debug-b7c7c7a.log.gz` |
...
- C1-C3 与 X1-X3 都由独立顶层 CLI session 产生，不是 hook 脚本直接 stdin 模拟；其协议代码从
  `68f1d43` 到 `b7c7c7a` 未变化；`.claude/settings.json` 的差异只增加 #16 Bash permission
  规则，未改变 SessionStart matcher。G1/G2 因 guard/parser 代码在 exact-head review 后变化，
  已在 `b7c7c7a` 的三个新 disposable clones 中重跑。
...
- 首次 G1 override 尝试（session `5a29d38b-4040-47a6-bbc0-a2ab4c6ca718`）在 tool call 前由
