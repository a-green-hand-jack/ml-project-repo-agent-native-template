...
> `22d7572607c7708a4d35da09aa62d5afa877b008`。
> 所有 session 都在 `/tmp/issue-13-18-fresh.UBWQdD/` 或
> `/tmp/issue13-g12-22d7572.LuxoWD/` 下的隔离 HOME 与 disposable clone 中启动；未修改真实
...
| G1 | Claude Code 2.1.207 · Opus 4.8 | reject `fede78ef-4218-4f5b-884e-076139f1fcf0`; skip `0234581e-bb31-4fbf-818a-68228bedf254` | PASS（目标 `22d7572`）：两份 raw 都先记录 exact HEAD、空 pre-status 与 registry present；同一 malformed-draft Write 在默认 session 被 `doc-lifecycle:` deny，post-status 仍空且 probe missing；独立 wrapper 在启动 Claude 前只增设进程级 `DOC_LIFECYCLE_SKIP=1`，唯一 Write 的 hook exit 0，post-status 只含该 probe，raw 同时记录 probe SHA256/正文与 registry present | `raw/13-G1-reject-claude-22d7572.typescript.gz`, `raw/13-G1-reject-claude-debug-22d7572.log.gz`, `raw/13-G1-skip-claude-22d7572.typescript.gz`, `raw/13-G1-skip-claude-debug-22d7572.log.gz` |
| G2 | Claude Code 2.1.207 · Opus 4.8 | `62ea9a4e-62ae-49bd-a43e-4fd7a622e94c` | PASS（目标 `22d7572`）：raw 先记录 exact HEAD、空 pre-status、registry present，以及 opaque file 为 26 bytes、SHA256 `6432e13b...613a9`、正文 `memory/doc-lifecycle.yaml`；唯一 Bash `git rm --pathspec-from-file=<file>` 被 deny；post-status 仍空、registry present、pathspec SHA256 未变 | `raw/13-G2-delete-claude-22d7572.typescript.gz` + `raw/13-G2-delete-claude-debug-22d7572.log.gz` |
...
- C1-C3 与 X1-X3 都由独立顶层 CLI session 产生，不是 hook 脚本直接 stdin 模拟；其协议代码从
  `68f1d43` 到 `22d7572` 未变化；`.claude/settings.json` 的差异只增加 #16 Bash permission
  规则，未改变 SessionStart matcher。G1/G2 因 guard/parser 代码在 exact-head review 后变化，
  已在 `22d7572` 的三个新 disposable clones 中重跑；wrapper 的 pre/post shell 输出与顶层 Claude
...
G1 raw wrapper: EVIDENCE_HEAD=22d7572...; reject post-status empty/probe missing;
