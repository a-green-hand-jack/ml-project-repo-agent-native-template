...
> `492e042f8c877cecfafbccc0e4536a391d855821`。
> 所有 session 都在 `/tmp/issue-13-18-fresh.UBWQdD/` 或
> `/tmp/issue13-g12-492e042.Xmbs5p/` 下的隔离 HOME 与 disposable clone 中启动；未修改真实
...
| G1 | Claude Code 2.1.207 · Opus 4.8 | reject `38dd6d17-b04a-40d3-bcb6-287cf875afc7`; skip `7eac2537-6bf1-4f61-bba4-17f9b23cbbe9` | PASS（目标 `492e042`）：默认 fresh session 的真实 Write 被 `doc-lifecycle:` 拒绝且 clone 保持 clean；独立 fresh session 仅设置 `DOC_LIFECYCLE_SKIP=1` 后同一无效 Write 成功 | `raw/13-G1-reject-claude-492e042.typescript.gz`, `raw/13-G1-reject-claude-debug-492e042.log.gz`, `raw/13-G1-skip-claude-492e042.typescript.gz`, `raw/13-G1-skip-claude-debug-492e042.log.gz` |
| G2 | Claude Code 2.1.207 · Opus 4.8 | `5e672f24-0b6c-4a15-8048-f68f75d40de4` | PASS（目标 `492e042`）：真实 Bash `env -C memory rm doc-lifecycle.yaml` 被 `doc-lifecycle: 禁止删除/移走` 拒绝；独立 disposable clone 保持 clean、注册表仍存在 | `raw/13-G2-delete-claude-492e042.typescript.gz` + `raw/13-G2-delete-claude-debug-492e042.log.gz` |
...
- C1-C3 与 X1-X3 都由独立顶层 CLI session 产生，不是 hook 脚本直接 stdin 模拟；其协议代码从
  `68f1d43` 到 `492e042` 未变化；`.claude/settings.json` 的差异只增加 #16 Bash permission
  规则，未改变 SessionStart matcher。G1/G2 因 guard/parser 代码在 exact-head review 后变化，
  已在 `492e042` 的三个新 disposable clones 中重跑。
