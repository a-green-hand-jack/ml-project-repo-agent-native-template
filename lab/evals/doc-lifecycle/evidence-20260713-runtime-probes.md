...
> 集成 main、修复 exact-head review 两条 MAJOR 后的 guard exact target：
> `3eb7b1a3f3bde2a0114930939d5293581da8832e`。
> 所有 session 都在 `/tmp/issue-13-18-fresh.UBWQdD/` 或
> `/tmp/issue13-g12-3eb7b1a.j6Erk7/` 下的隔离 HOME 与 disposable clone 中启动；未修改真实
...
| G1 | Claude Code 2.1.207 · Opus 4.8 | reject `47886781-3f73-4cf6-a6d0-da64175ac925`; skip `0605199f-c615-445b-988f-dcb5f330deb0` | PASS（目标 `3eb7b1a`）：默认 fresh session 的真实 Write 被 `doc-lifecycle:` 拒绝且 clone 保持 clean；独立 fresh session 仅设置 `DOC_LIFECYCLE_SKIP=1` 后同一无效 Write 成功 | `raw/13-G1-reject-claude-3eb7b1a.typescript.gz`, `raw/13-G1-reject-claude-debug-3eb7b1a.log.gz`, `raw/13-G1-skip-claude-3eb7b1a.typescript.gz`, `raw/13-G1-skip-claude-debug-3eb7b1a.log.gz` |
| G2 | Claude Code 2.1.207 · Opus 4.8 | `9fa3f94a-5e43-47bc-8ddb-60b82f62687a` | PASS（目标 `3eb7b1a`）：真实 Bash `env -C memory rm doc-lifecycle.yaml` 被 `doc-lifecycle: 禁止删除/移走` 拒绝；独立 disposable clone 保持 clean、注册表仍存在 | `raw/13-G2-delete-claude-3eb7b1a.typescript.gz` + `raw/13-G2-delete-claude-debug-3eb7b1a.log.gz` |
...
- C1-C3 与 X1-X3 都由独立顶层 CLI session 产生，不是 hook 脚本直接 stdin 模拟；其协议代码从
  `68f1d43` 到 `3eb7b1a` 未变化；`.claude/settings.json` 的差异只增加 #16 Bash permission
  规则，未改变 SessionStart matcher。G1/G2 因 guard/parser 代码在 exact-head review 后变化，
  已在 `3eb7b1a` 的三个新 disposable clones 中重跑。
