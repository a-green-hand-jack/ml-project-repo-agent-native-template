...
代码未再变化。exact-head review 后改变的 guard G1/G2 已在精确目标 `b7c7c7a` 的新 disposable
...
| G1 | 2026-07-13 | Claude Code 2.1.207 / Opus 4.8 | `b7c7c7a` | PASS | `raw/13-G1-*-b7c7c7a.*`; 同一 malformed-draft payload 默认 deny，独立 SKIP session allow；首次模型拒绝 raw 保留但不计 PASS | Codex integration owner |
| G2 | 2026-07-13 | Claude Code 2.1.207 / Opus 4.8 | `b7c7c7a` | PASS | `raw/13-G2-*-b7c7c7a.*`; opaque `--pathspec-from-file` 真载荷被 deny，注册表仍存在 | Codex integration owner |
