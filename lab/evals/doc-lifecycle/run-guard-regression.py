...
def main() -> int:
    ...
    cases = [
        ...
        ("final-review-2: cp existing target directory overwrite registry 拦", "Bash",
         {"command": "cp /tmp/doc-lifecycle.yaml memory"}, 2, None),
        ("final-review-2: cp -t overwrite registry 拦", "Bash",
         {"command": "cp -t memory /tmp/doc-lifecycle.yaml"}, 2, None),
        ("final-review-2: cp -tmemory overwrite registry 拦", "Bash",
         {"command": "cp -tmemory /tmp/doc-lifecycle.yaml"}, 2, None),
        ("final-review-2: cp -vt overwrite registry 拦", "Bash",
         {"command": "cp -vt memory /tmp/doc-lifecycle.yaml"}, 2, None),
        ("final-review-2: cp --target-directory value overwrite registry 拦", "Bash",
         {"command": "cp --target-directory memory /tmp/doc-lifecycle.yaml"}, 2, None),
        ("final-review-2: cp --target-directory=value overwrite registry 拦", "Bash",
         {"command": "cp --target-directory=memory /tmp/doc-lifecycle.yaml"}, 2, None),
        ("exact-head-review: cp --t abbreviation overwrite registry 拦", "Bash",
         {"command": "cp --t=memory /tmp/doc-lifecycle.yaml"}, 2, None),
        ("exact-head-review: cp --ta abbreviation overwrite registry 拦", "Bash",
         {"command": "cp --ta memory /tmp/doc-lifecycle.yaml"}, 2, None),
        ("exact-head-review: cp target $PWD expansion overwrite registry 拦", "Bash",
         {"command": "cp --target-directory=$PWD/memory /tmp/doc-lifecycle.yaml"}, 2, None),
        ("exact-head-review: cp direct ${PWD} expansion overwrite registry 拦", "Bash",
         {"command": "cp /tmp/doc-lifecycle.yaml ${PWD}/memory"}, 2, None),
        ("exact-head-review: cp unknown shell expansion fail-closed", "Bash",
         {"command": "cp --target-directory=$DEST /tmp/doc-lifecycle.yaml"}, 2, None),
        ("exact-head-review: cp --parents abbreviation overwrite registry 拦", "Bash",
         {"command": "cd scripts && cp --pa --t=.. memory/doc-lifecycle.yaml"}, 2, None),
        ("exact-head-review 2: rm $PWD registry 拦", "Bash",
         {"command": "rm $PWD/memory/doc-lifecycle.yaml"}, 2, None),
        ("exact-head-review 2: rm ${PWD} registry 拦", "Bash",
         {"command": "rm ${PWD}/memory/doc-lifecycle.yaml"}, 2, None),
        ("exact-head-review 2: mv $PWD registry 拦", "Bash",
         {"command": "mv $PWD/memory/doc-lifecycle.yaml /tmp/away"}, 2, None),
        ("exact-head-review 2: dd of=$PWD registry 拦", "Bash",
         {"command": "dd if=/dev/null of=$PWD/memory/doc-lifecycle.yaml"}, 2, None),
        ("exact-head-review 2: tee $PWD registry 拦", "Bash",
         {"command": "printf x | tee $PWD/memory/doc-lifecycle.yaml"}, 2, None),
        ("exact-head-review 2: git rm $PWD registry 拦", "Bash",
         {"command": "git rm $PWD/memory/doc-lifecycle.yaml"}, 2, None),
        ("exact-head-review 2: redirect $PWD registry 拦", "Bash",
         {"command": "printf x > $PWD/memory/doc-lifecycle.yaml"}, 2, None),
        ("exact-head-review 2: rm 未知展开 fail-closed", "Bash",
         {"command": "rm $DEST/doc-lifecycle.yaml"}, 2, None),
        ...
        ("control: repo 外 cp -t overwrite 放行", "Bash",
         {"command": "cp -t /tmp/not-this-repo/memory /tmp/doc-lifecycle.yaml"}, 0, None),
        ("control: repo 外 cp --target-directory overwrite 放行", "Bash",
         {"command": "cp --target-directory=/tmp/not-this-repo/memory /tmp/doc-lifecycle.yaml"},
         0, None),
        ("control: repo 外 cp --t abbreviation overwrite 放行", "Bash",
         {"command": "cp --t=/tmp/not-this-repo/memory /tmp/doc-lifecycle.yaml"}, 0, None),
        ("control: cp $PWD expansion 指向非 registry 放行", "Bash",
         {"command": "cp --target-directory=$PWD/not-memory /tmp/doc-lifecycle.yaml"}, 0, None),
        ("control: single-quoted $PWD 保持 literal 放行", "Bash",
         {"command": "cp /tmp/doc-lifecycle.yaml '$PWD/memory'"}, 0, None),
        ("control: escaped $PWD 保持 literal 放行", "Bash",
         {"command": r"cp /tmp/doc-lifecycle.yaml \$PWD/memory"}, 0, None),
        ("control: rm $PWD 指向非 registry 放行", "Bash",
         {"command": "rm $PWD/not-memory/doc-lifecycle.yaml"}, 0, None),
        ("control: dd of=$PWD 指向非 registry 放行", "Bash",
         {"command": "dd if=/dev/null of=$PWD/not-memory/doc-lifecycle.yaml"}, 0, None),
        ("control: rm single-quoted $PWD literal 放行", "Bash",
         {"command": "rm '$PWD/memory/doc-lifecycle.yaml'"}, 0, None),
        ("control: rm escaped $PWD literal 放行", "Bash",
         {"command": r"rm \$PWD/memory/doc-lifecycle.yaml"}, 0, None),
    ...
    try:
        ...
        for name, command in (
            ...
            ("cp existing target directory overwrite", "cp /tmp/doc-lifecycle.yaml memory"),
            ("cp -t overwrite", "cp -t memory /tmp/doc-lifecycle.yaml"),
            ("cp --target-directory overwrite",
             "cp --target-directory=memory /tmp/doc-lifecycle.yaml"),
            ("cp --t abbreviation overwrite", "cp --t=memory /tmp/doc-lifecycle.yaml"),
            ("cp target $PWD expansion overwrite",
             "cp --target-directory=$PWD/memory /tmp/doc-lifecycle.yaml"),
            ("cp unknown shell expansion fail-closed",
