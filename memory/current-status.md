...
| `git commit -m "test: add Agent-R1 adoption replay case"` | case 内容提交成功：`5c8850f`。 |
| `git commit -m "docs: record Agent-R1 case metadata"` | case 元数据修正提交成功：`a83b870`。 |
| final post-commit matrix | `check-adoption-integrity.py .`、`run-adoption-smoke.py`、`validate-governance --strict`、`git diff --check` 全通过；worktree clean。 |
...
- 无当前 blocker。Case branch 已提交并通过 post-commit pressure matrix。
- 后续压力测试应检查：validator/harness 是否仍通过；case 内容是否触发 tracked bytes
  或 root pollution；adoption integrity 是否可复验。

## Exact next steps

1. 保留 `worktree-case+agent-r1-adoption-replay` 作为 template repo 的 Agent-R1 case branch。
2. 若要远端保存，push topic branch（非 main；仍按 human gate）。
3. 不要默认把完整 Agent-R1 case 内容合回 `main`。
