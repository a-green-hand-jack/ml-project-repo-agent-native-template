...
- evidence-only `83be6d5` 的独立 final review 已完成并给出 `CHANGES_REQUESTED`：GNU
  `cp -t/--target-directory` 可绕过 registry overwrite guard，且 plan/checklist/status 的 durable
  记录仍有矛盾。前一 code target `22d7572` 的 G1/G2 raw evidence 仍可核验，但不能冒充本轮
  修复的最终证据。当前正在修复这两项 MAJOR、补 normal/`python -S` 对抗 fixtures；修复提交后
  必须在精确新 code target 重跑三个独立顶层 Claude G1/G2 sessions、全 strict gates 与独立
  final fresh review。仅 `APPROVE` 后才可合入，期间保持 implementing。
...
- #13：C1-C3/X1-X3 真实 runtime evidence 有效；evidence-only `83be6d5` 的独立 final review
  因 GNU `cp -t/--target-directory` overwrite 绕过与 durable 状态矛盾给出 `CHANGES_REQUESTED`。
  当前修复完成后仍须在精确新 code target 重跑 G1/G2、全 strict gates，并让 direct evidence
  child 取得独立 fresh `APPROVE`，否则不得合入。
