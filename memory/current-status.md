...
- **A5 的 fresh-Codex-session runtime smoke 未执行**：plan A5 要求「从 bootstrapped repo 启动一个
  fresh Codex session，记录 guidance/skill discovery 与 project hook 加载的最小证据」。这需要一个
  真实 Codex CLI session 对着 bootstrapped repo 跑，Claude subagent 无法代为执行——只完成了 A5 的
  synthetic-fixture 部分（`lab/evals/bootstrap/run-bootstrap-smoke.py` + 三个 static validator 全绿，
  证明「静态自洽」）。这条运行时证据仍是待办，需要 human 或 Codex session 后续补齐。
...
2. 找机会从一个真正 bootstrapped 的 repo 里跑一次 fresh Codex session，补齐 A5 缺的 runtime smoke
   证据（guidance/skill discovery + project hook 加载 provenance）。
