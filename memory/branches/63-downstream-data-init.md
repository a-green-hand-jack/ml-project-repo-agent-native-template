...
## Fresh review round 1（MAJOR-1，已修复）

fresh reviewer 结论：REQUEST CHANGES，1 个 MAJOR，A/C/D/E/F/G 全过，自报的 B1/B2/B3 三个决策
均被裁定成立（B1 范围扩展方向正确但实现漏了一处；B2 例外够窄；B3 不 bump 正确）。

**MAJOR-1**：`validate-governance.py::check_evidence_chain()` 里第 208 行「`status∈{partial,
supported}` 但无 evidence 支撑」已加 `and not is_legacy_claim` 豁免，但紧邻第 210 行「
`status==supported` 且最强证据低于 metric」这条更严检查漏了同样的豁免。后果：`supported` 且
全部 evidence 已降级为 legacy 的 claim——`init_claims()` 会标记它、`check-provenance-chain`
豁免它、唯独 `validate-governance` 恒 FAIL——三方语义不一致，破坏 D3「init 后 G1 全绿」不变式。
ELF fixture 里没有 `supported` 状态的 claim（4 条全是 `partial`），所以自验没暴露；reviewer
用自构探针实证（init 标记 legacy=True / provenance 0 fail / governance FAIL overclaim）。

**修复**（reviewer 首选方案，与第 208 行、`check-provenance-chain.py` 的三态判定对齐）：
`scripts/validate-governance.py:210` 补 `and not is_legacy_claim`。

**固化为回归**（`scripts/init-governance-data.py::_self_test()` 新增独立 tmp fixture 块，
单一 owner，通过 `importlib` 同时加载 `check-provenance-chain.py` 与 `validate-governance.py`
两个真实模块——不是 mock）：
- 正例：`claim-supported-legacy`（status=supported，唯一 evidence grade=metric 但缺
  run_id/config）跑真实 `run(tmp2)` 后，断言 evidence 与 claim 都被标记
  `governance_status: legacy_unverified`；断言 `check-provenance-chain.check_evidence`/
  `run_checks` 对它 0 fail；断言 `validate-governance.check_evidence_chain()` 对它 0 error。
- 负例：`claim-supported-weak`（同样 status=supported，但**无** legacy 标记，evidence 字段
  齐全、只是 grade=log < metric）——断言 `validate-governance` 仍报
  `overclaim：... 最强证据低于 metric`，证明豁免范围没有放松正常判定。
- 已用「临时还原 sed 掉 `and not is_legacy_claim`」的方式验证过这条回归确实会失败（捕获到
  reviewer 描述的具体错误消息），再恢复修复重跑转绿，确认回归不是假阳性。

## Latest result

D1–D3 全部实现完成，D2 验收路径全部通过（非 strict 口径，对应 D3 明文验收标准），MAJOR-1 已按
reviewer 首选方案修复并固化为回归，ELF worktree 事故已按派发方指定顺序完整修复并三重验证。
delta re-review 前的全套复跑（4 个 `--self-test`、`validate-governance --strict`、
`check-agent-harness --strict`、`check-anatomy-drift`、`sync-codex-adapters --check`、
三个 smoke、`py_compile`、独立 `git clone` fixture 上 init→全绿→幂等）全部 PASS，见下方
「D2 验收路径」（步骤 1–13，MAJOR-1 修复后在新 fixture `/tmp/elf-fixture-63-v3` 上重验，结果
与首次一致：baseline `1/23/28` FAIL、init `changed=34`、init 后全绿、二次运行 `changed=0`）。

## Open risks

- ~~`validate-governance.py::check_evidence_chain()` 的 legacy 豁免是范围扩展~~——fresh review
  已确认扩展方向正确（B1 成立），且已修复该扩展实现中遗漏的第二处豁免点（MAJOR-1）。
- artifact-index 的 `location` 空值原本是「任何真实状态都不允许跳过」的硬编码无例外规则；
  本次新增了 legacy 例外——fresh review 已确认这个例外足够窄（B2 成立）：只对显式登记
  `governance_status: legacy_unverified` + 非占位 `governance_note` 的条目生效，新条目/无
  标记条目判定丝毫未变松。
- `contract_version` 保持 2 未 bump——fresh review 已确认这个判级正确（B3 成立）：新增 TS-11
  属 receipt schema 增字段，MINOR，不需要 bump，与 #67 分支「新增 CLI 参数」bump 的场景不是
  同一类变更。
...
实现完成、D1–D3 自验全绿、MAJOR-1 已修复并固化为回归、ELF worktree 事故已修复验证、branch
status 已落盘。下一步：原子 commit（新 commit，未 amend，保留 review 轨迹）+ push topic
分支（已获授权）；等待 delta re-review；PR/merge/关闭 issue 均为 human gate，本 agent 不代做。
