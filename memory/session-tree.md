...
| issue-12-smoke-contract | Issue #12 part C: unified runtime/smoke verification contract for `adopt-existing-repo.py` `prove` phase + `check-adoption-integrity.py` (schema, decoupled exit code, real-repo replay) | `feat/12c-smoke-contract` / `.claude/worktrees/12c-smoke-contract` (branched from `feat/12-bootstrap-adoption-proof`, so it already carries part A) | `plans/20260712-bootstrap-adoption-proof.zh.md` (task tree C + D items owned by C) | impl done, awaiting human review/merge | Human review + decide on push/PR gate; then start issue #12 part B (semantic classification) in its own worktree, independent of this branch. |
