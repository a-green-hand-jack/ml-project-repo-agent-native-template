...
> 建立心智模型。
>
> **这份文档不是什么**：不是第二套 doctrine。真正的规则住在 `.agent/*.md`、`.claude/`、`scripts/`；
> 本文与它们冲突时**以源文件为准**。本文只做地图与解释，尽量指向源文件而非复制其内容。
>
> **来源**：设计精神与实践依据是 `.reference-docs/` 两份文档（spirit + practice）。本文是它们在本
> repo 的落地说明。
...
Enforcement 层（机器强制）    ──  .claude/settings.json · .claude/hooks/*.py · .claude/statusline.sh
Validation 层（证明没漂移）   ──  scripts/*.py · .github/workflows/governance.yml
人机接口层（human 可信信息也进 repo）── human/ · memory/ · plans/ · DECISIONS.md
```

**铁律：三层必须对齐。** doctrine 说「为什么」，settings+hooks「机器执行」，scripts「验证没失配」。
...
| 能力 | `.claude/` | subagents / skills / commands / hooks / settings / statusline |
...
| --- | --- | --- |
| **permission**（allow/ask/deny，`settings.json` + `settings.local.json`） | **可调** | 放宽，不打断低风险 |
...
### 3.1 permission 三档（`settings.json`）
...
- **allow**（~115 条）：只读 shell、只读 git、开发工具、`uv run`、`python -c`、`Edit/Write`（受保护路径除外）、可逆 git（add/commit/stash）、清缓存目录。
...
它拦：提权/远程执行、`pip install`、`rm -r` **按目标分级**（缓存/构建/临时可删；数据·产物·`.git`·绝对路径·仓库根·`..` 拦；`find -exec` 嵌套有兜底正则）、`mv/cp/rsync/dd` 触碰受保护路径、受保护路径的 `Edit/Write`、push 到 `main/master`（除非 `CLAUDE_ALLOW_PUSH_MAIN=1`）。
...
human 出门/睡觉但任务要跑时：`cp` 出 `settings.local.json`（git-ignored）临时放宽 `ask→allow`，
用完 `rm` 收回。permission 放宽而 hook 地板照守，所以「授权自主」安全。

### 3.4 副作用半径与人机门禁

选动作按「错了会怎样」判断（`action-boundary.md` §副作用半径）。外部/不可逆动作走 `human-gates.md`：
agent **准备**（写好 commit、起草 PR body、生成归档提案），human **触发**。
机器层是 `settings.json` 的 ask/deny + hook；流程层是 `pull_request_template.md` + `CODEOWNERS`；记录层是 `human/decisions/`。
...
- 15 个 subagent 定义在 `.claude/agents/`，全部 `model: inherit`——**不写死模型**。
- 因为「预算不是身份」：模型/effort 由**任务形状**决定，不由角色名决定。
...
| 类别 | 位置 | 数量 | 说明 |
| --- | --- | --- | --- |
| Subagents | `.claude/agents/` | 15 | 执行(5) / 协调(2) / 维护交互(5) / 演化(3)，全 `model: inherit` |
| Skills | `.claude/skills/` | 8 | worktree-pr-flow / experiment-workflow / artifact-indexing / session-boundary-control / subagent-routing / interactive-plan-doc / anatomy-drift-control / workflow-recipe-harvesting |
| Commands | `.claude/commands/` | 7 | checkpoint / experiment-watch / feature-split / paper-reproduce / pr-review / result-promote / weekly-maintenance |
| Hooks | `.claude/hooks/` | 3 | pre_tool_guard（地板）/ pre_compact_memory_check（提醒）/ subagent_report_index（记录） |
| Validators | `scripts/` | 4 | check-agent-harness / check-anatomy-drift / validate-governance / check-same-commit |
| Doctrine | `.agent/` | 21 md | 索引在 `.agent/AGENTS.md`（读取顺序 1–15） |
...
- **能力 repo-local**：项目相关 agent/skill/command/hook 放 `.claude/`，不装 user 全局。
...
| subagent `model: inherit`，不写死 | 预算是任务属性不是角色属性；写死会把成本绑在名字上，且无法校准 |
...
| 两份 `.reference-docs` 保留在 repo | doctrine 有版本、可 review；派生项目可保留或更新自己信奉的版本 |
...
- **想改治理/权限**：`.agent/action-boundary.md` + `.claude/settings.json` + `scripts/`（三者一起改）。
