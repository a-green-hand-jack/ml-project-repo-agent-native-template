...
- **provenance 链机器可检查**（`check-provenance-chain.py`，作为 `validate-governance.py` 子检查）：run→artifact→evidence→claim→deliverable 引用完整性、run 闭环（ledger `done`+`run_summary`）、checksum（统一 sha256，无法校验需固定枚举 reason + 非占位理由）、deliverable Markdown 的 claim marker（`<!-- claim: id=... -->`）。三态输出（pass/fail/unknown），unknown 不算 pass。覆盖全部 7 类 index（result/table/figure/trace/model/checkpoint/dataset，统一 `location` 字段 + `schema_version`；`commit`/`config`/`run_id` 三元组统一必填，确无 run 来源须显式豁免——`provenance_unavailable_reason` 固定枚举 + 非占位理由）与 release gate 的 `structured_checks`（只结构化可机械验证的 kind；结果仅建议信号，`gate_status` 翻转仍是 human 动作，唯一 FAIL 情形是 passed 却有检查不满足；`artifact-exists` 查 location 文件/manifest 记录真实存在，`checksum-verified` 只认真算 sha256 比对通过，waived ≠ verified）。活跃交付物（「evidence 齐全=是」且非 draft）要求正文 claim marker 或 `human/reviews/results/` 人工 review 证据二选一。
