...
> **canonical 声明式来源是 `.agent/capability-catalog.toml`**（见 issue #28）：每个正式
> agent/skill/command/hook 在那里登记 inputs/outputs/validators/human gates/path
> boundaries/adapters/completion contract，**exact member set 由
> `scripts/check-capability-catalog.py`（已并入 `validate-governance`）机器强制**——登记
> 齐全 / 无幽灵条目 / adapter 内容 parity。下表只保留**分类计数 + 指向 catalog 的链接**，
> 不手工重复成员名单；数量由 `scripts/check-agent-harness.py` 校验（见文末维护说明）。
> 注：Hooks 行的 11 是 `.claude/hooks/*.py` 物理文件数；catalog 登记的是**真实注册**的 8 个
> hook（helper/setter 不算正式 hook capability，以 settings/config 挂载面为准）。

| 类别 | 位置 | 数量 | 说明 |
| --- | --- | --- | --- |
| Subagents | `.claude/agents/` | 16 | 执行(6) / 协调(2) / 维护交互(5) / 演化(3)，除 `zh-review-gate`（锁定 `haiku`）外全 `model: inherit` |
| Skills | `.claude/skills/` | 14 | worktree-pr-flow / experiment-workflow / artifact-indexing / session-boundary-control / subagent-routing / interactive-plan-doc / anatomy-drift-control / workflow-recipe-harvesting / template-stress-test / adopt-existing-repo / coding-agent-quota / template-feedback / spawn / bootstrap-project |
...
| Hooks | `.claude/hooks/` | 11 | pre_tool_guard（安全地板 + launch gate + 多 agent 冲突/写错-worktree 薄接线）/ format_changed_python（提醒）/ pre_compact_memory_check（提醒）/ subagent_report_index（记录）/ zh_review_advisory（提醒）/ context_threshold_notice（阈值注入）/ context_continuity（compact 后回注）/ agent_identity_hook（自命名指令+自知注入）/ context_usage（helper）/ agent_identity（身份解析 helper）/ agent_name_set（自命名 setter：rename+roster+控制面状态） |
| Codex adapters | `.codex/` `.agents/` | 16 agents + 22 skills | `.codex/agents` custom-agent adapters；`.agents/skills` 包含 14 个 workflow skill（含 `bootstrap-project`，不生成 command-* skill）+ 8 个 command adapter |
| Validators / tools | `scripts/` | 17 | check-agent-harness / check-anatomy-drift / check-provenance-chain / check-capability-catalog / validate-governance / validate-experiment-state / check-same-commit / adopt-existing-repo / check-adoption-integrity / bootstrap-project / sync-codex-adapters / bump-template-version / template-sync / agent-state / agent-status / agent-mailbox / check-agent-conflicts |
| Doctrine | `.agent/` | 22 md | 索引在 `.agent/AGENTS.md`（读取顺序 1–18，另含 `AGENTS.md`/`README.md`/`agent-identity.md` 本身） |
...
- **新能力先在 `.agent/capability-catalog.toml` 登记 contract 再写实现**；没有登记的能力不算正式 surface（`tool-skill-interface.md`），`check-capability-catalog.py` 会因 missing 而失败。
