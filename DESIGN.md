...
> boundaries/adapters/completion contract，并由 `scripts/check-capability-catalog.py`
> （已并入 `validate-governance`）机器强制「登记齐全 / 无幽灵条目 / adapter parity」。
> 下表只是给 human 读的**派生摘要**，不再是唯一不可执行清单；数量由
> `scripts/check-agent-harness.py` 校验（见文末维护说明）。
