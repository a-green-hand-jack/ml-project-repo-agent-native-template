...
| Skills | `.claude/skills/` | 11 | worktree-pr-flow / experiment-workflow / artifact-indexing / session-boundary-control / subagent-routing / interactive-plan-doc / anatomy-drift-control / workflow-recipe-harvesting / template-stress-test / adopt-existing-repo / coding-agent-quota |
...
| Hooks | `.claude/hooks/` | 5 | pre_tool_guard（地板）/ format_changed_python（提醒）/ pre_compact_memory_check（提醒）/ subagent_report_index（记录）/ zh_review_advisory（提醒） |
| Codex adapters | `.codex/` `.agents/` | 16 agents + 19 skills | `.codex/agents` custom-agent adapters；`.agents/skills` 包含 11 个 workflow skill + 8 个 command adapter |
| Validators / tools | `scripts/` | 7 | check-agent-harness / check-anatomy-drift / validate-governance / check-same-commit / adopt-existing-repo / check-adoption-integrity / sync-codex-adapters |
