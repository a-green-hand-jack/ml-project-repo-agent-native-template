...
> 注：Hooks 行的 12 是 `.claude/hooks/*.py` 物理文件数；catalog 登记的是**真实注册**的 9 个
...
| Skills | `.claude/skills/` | 14 | repo-local workflow；成员集权威在 `.agent/capability-catalog.toml`（本表不再手工枚举） |
| Commands | `.claude/commands/` | 8 | Claude slash command；成员集见 catalog |
| Hooks | `.claude/hooks/` | 12 | Claude/Codex 共享强制/提醒地板；12 = 物理 `.py` 文件数，catalog 登记 9 个**真实注册** hook（helper/setter 不算），成员集见 catalog |
...
| Validators / tools | `scripts/` | 18 | check-agent-harness / check-codex-hook-runtime / check-anatomy-drift / check-provenance-chain / check-capability-catalog / validate-governance / validate-experiment-state / check-same-commit / adopt-existing-repo / check-adoption-integrity / bootstrap-project / sync-codex-adapters / bump-template-version / template-sync / agent-state / agent-status / agent-mailbox / check-agent-conflicts |
