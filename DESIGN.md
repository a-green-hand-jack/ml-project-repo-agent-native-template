...
| Skills | `.claude/skills/` | 12 | worktree-pr-flow / experiment-workflow / artifact-indexing / session-boundary-control / subagent-routing / interactive-plan-doc / anatomy-drift-control / workflow-recipe-harvesting / template-stress-test / adopt-existing-repo / coding-agent-quota / template-feedback |
...
| Hooks | `.claude/hooks/` | 9 | pre_tool_guard（地板）/ format_changed_python（提醒）/ pre_compact_memory_check（提醒）/ subagent_report_index（记录）/ zh_review_advisory（提醒）/ context_threshold_notice（阈值注入）/ context_continuity（compact 后回注）/ context_usage（共用 helper）/ agent_identity（身份解析 helper，非注册 hook） |
| Codex adapters | `.codex/` `.agents/` | 16 agents + 20 skills | `.codex/agents` custom-agent adapters；`.agents/skills` 包含 12 个 workflow skill + 8 个 command adapter |
| Validators / tools | `scripts/` | 9 | check-agent-harness / check-anatomy-drift / validate-governance / check-same-commit / adopt-existing-repo / check-adoption-integrity / sync-codex-adapters / bump-template-version / template-sync |
