...
- 16 个 subagent 定义在 `.claude/agents/`，除 `zh-review-gate`（刻意锁定廉价模型，见其
...
| Skills | `.claude/skills/` | 9 | worktree-pr-flow / experiment-workflow / artifact-indexing / session-boundary-control / subagent-routing / interactive-plan-doc / anatomy-drift-control / workflow-recipe-harvesting / template-stress-test |
...
| Hooks | `.claude/hooks/` | 4 | pre_tool_guard（地板）/ pre_compact_memory_check（提醒）/ subagent_report_index（记录）/ zh_review_advisory（提醒） |
| Validators | `scripts/` | 4 | check-agent-harness / check-anatomy-drift / validate-governance / check-same-commit |
| Doctrine | `.agent/` | 19 md | 索引在 `.agent/AGENTS.md`（读取顺序 1–16，另含 `AGENTS.md`/`README.md` 本身） |
