...
4. 若使用 Codex，信任本 repo 的 `.codex/` project config；改 `.claude/` 能力后运行
   `python scripts/sync-codex-adapters.py` 同步 `.codex/` 与 `.agents/`。
5. 删掉用不到的目录（模板是「一次建好，按需删减」，不是「一定全用」）。
6. 跑 `python scripts/validate-governance.py` 确认 harness 仍然自洽。
7. 在 `.reference-docs/` 里保留或更新你信奉的 doctrine 版本。
