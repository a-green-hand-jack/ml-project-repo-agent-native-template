...
7. 写入边界：`--ledger` / `--record-ledger` 的**写入**目标 resolve(realpath) 后只允许
   默认 `.outcome-ledger/` 目录内或**字面 `/tmp` 前缀**（POSIX 标准位置；刻意不读
   `tempfile.gettempdir()`——`TMPDIR` 环境可控，绝不据此放宽 allowlist）；测试脚本
   若 temp 不在 `/tmp` 下，用显式 `--allow-test-dir <path>`（仅测试用，不从环境推导，
   受保护地板仍然优先）。受保护路径（`lab/data|runs|models`、`lab/infra/private`、
   `checkpoints`、`wandb`、`mlruns`、`.env`，对齐 pre_tool_guard）与其他任意路径一律
   拒绝并以非零退出。读取路径不受限（fixtures 可读）。已知限制（诚实记录）：resolve
   与 open 之间存在 TOCTOU 窗口，本边界与 pre_tool_guard 同属防误操作护栏而非对抗性
   沙箱，数据安全最终靠 gitignore + 备份。
