...
- **具体路线隔离**：统计与推荐证据以 `provider + model + effort + role + task_class +
  routing_tier + policy_version` 完整键分桶；一个 Codex model/effort 的结果不能为另一个
  具体 Codex 路线背书，旧 policy 的结果也不能穿透到新 policy。
- **ID 唯一**：decision 的 `decision_id` 在 ledger 内唯一；两个写入口都在 append 前读取并
  拒绝重复 ID，避免确定性 replay 重复落盘自污染。
...
6. required key 缺失 → 拒绝：decision / outcome 各有完整 required-key 清单
   ...
   null）；`schema_version` 必须是精确的 int 1（`true`/`1.0`/`"1"` 等类型混淆值
   一律拒绝——Python 里 `True == 1 == 1.0`）；`decision_id` 必须匹配
   `d-<id>` 格式；`quota_cost` 若在场必须结构完整（`QUOTA_COST_REQUIRED_KEYS`），
   残缺即拒绝。负向回归：`scripts/check-outcome-ledger-schema.py` 逐个删 key 断言 FAIL。
7. 写入边界：`--ledger` / `--record-ledger` 的**写入**目标经 lexical + realpath 双重
   containment 检查后，只允许默认 `.outcome-ledger/` 目录内或**字面 `/tmp` 前缀**。
   生产 CLI 不提供额外目录 escape hatch，刻意不读 `tempfile.gettempdir()` / `TMPDIR`。
   repo 根、任何 `.env*` path component、symlink component、`..`/realpath 越界与其他任意
   路径均拒绝并以非零退出；读取路径不受限。已知限制：检查与 open 之间仍有 TOCTOU
   窗口，本边界是防误操作护栏而非对抗性 sandbox。
8. schema-invalid / 损坏 ledger 不参与路由：`outcome_route.py` 遇到 parse/schema
   错误（含非 UTF-8 坏字节、不可读文件——`read_ledger` 捕获 UnicodeDecodeError/
   OSError 走同一降级路径）时丢弃全部 ledger 证据，`degraded=true` 回退 quota-only
   推荐（缺数据保守回退），坏记录绝不进入统计。
9. `--min-samples` 必须为正整数（≥1）；零条 observed outcome（包括只有 pending decision）
   必须 `degraded=true` 并回退 quota-only，不得通过零阈值伪装成证据充分。
