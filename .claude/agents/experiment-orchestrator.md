...
- human 可在 alert 里落 approved_by/approved_at/approved_action 审计字段；它们不是防伪造的
  capability。你只能执行
  `python lab/infra/launch/expctl.py validate-recovery --run-id <id> --alert-id <aid>`
  只读校验 exact command/run/workdir/interpreter 与 pending 状态；actual apply-recovery fail-closed。
...
- 这些高风险动作只能产出「建议 + 待批准的确切命令」；`validate-recovery` 只是
