...
related_files:
  ...
  - skills/coding-agent-quota/schema.md
  - skills/coding-agent-quota/scripts/read_agent_quota.py
  - skills/coding-agent-quota/scripts/outcome_route.py
  - skills/coding-agent-quota/scripts/outcome_ledger.py
  - hooks/pre_tool_guard.py
  - hooks/format_changed_python.py
  - hooks/codex_runtime_receipt.py
...
| `skills/coding-agent-quota/` | quota snapshot + outcome-aware 路由：`read_agent_quota.py`（额度证据，行为不变）之上叠加 `outcome_route.py`/`outcome_ledger.py`（完整具体路线隔离、确定性 replay、零/薄样本 degraded 回退、唯一 `decision_id` 追溯、canonical ledger/`/tmp` 写边界）；schema 见 `schema.md`，词表冻结在 `fixtures/outcome/model-catalog.v1.json`，benchmark 冻结登记在 `benchmarks/`。 |
| `commands/` | slash command 定义。 |
| `hooks/pre_tool_guard.py` | `PreToolUse` 硬约束：拦截危险 Bash 与受保护路径写入；另薄接线 `scripts/check-agent-conflicts.py` 拦多 agent ownership 冲突/写错 worktree（`.agent/multi-agent-control-plane.md`）。 |
| `hooks/format_changed_python.py` | `PostToolUse` advisory：Claude（`.claude/settings.json` PostToolUse）与 Codex（`.codex/config.toml`）均调此脚本，从 hook 输入识别变更的 `.py` 文件并尝试 `ruff format`；ruff 缺失时打印可见 stderr 提示后放行（不阻断）。 |
| `hooks/codex_runtime_receipt.py` | Codex-only `SessionStart` 薄接线：调用 `scripts/check-codex-hook-runtime.py --write-receipt`，让 fresh session 对 project hooks 真加载产生可观察、gitignored 的 bundle receipt。 |
...
| `.claude/skills/coding-agent-quota/.outcome-ledger/ledger.jsonl` | `outcome_route.py --record` / `outcome_ledger.py` | 本地累积的 route decision / outcome 明细（gitignored；decision ID append 前查重；仅 README/.gitkeep 入 Git；schema 门禁 `scripts/check-outcome-ledger-schema.py`） |
