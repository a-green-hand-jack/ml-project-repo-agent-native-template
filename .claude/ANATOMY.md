...
| `skills/coding-agent-quota/` | quota snapshot + outcome-aware 路由：`read_agent_quota.py`（额度证据，行为不变）之上叠加 `outcome_route.py`/`outcome_ledger.py`（历史结果证据、确定性 replay、degraded 回退、`decision_id` 追溯）；schema 见 `schema.md`，词表冻结在 `fixtures/outcome/model-catalog.v1.json`，benchmark 冻结登记在 `benchmarks/`。 |
...
| `.claude/skills/coding-agent-quota/.outcome-ledger/ledger.jsonl` | `outcome_route.py --record` / `outcome_ledger.py` | 本地累积的 route decision / outcome 明细（gitignored，仅 README/.gitkeep 入 Git；schema 门禁 `scripts/check-outcome-ledger-schema.py`） |
