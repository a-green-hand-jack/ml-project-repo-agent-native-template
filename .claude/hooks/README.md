...
- 本地手测：`echo '{"tool_name":"Bash","tool_input":{"command":"sudo rm -rf /tmp/x"}}' | python3 .claude/hooks/pre_tool_guard.py; echo "exit=$?"`（应 exit 2）。注：`git push` 不再被 hook 硬拦，改由 settings.json `ask` 每次确认。`
