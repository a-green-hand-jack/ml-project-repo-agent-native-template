...
| 文件 | 事件 | 作用 | 阻止能力 |
| --- | --- | --- | --- |
| `pre_tool_guard.py` | `PreToolUse(Bash\|Edit\|Write)`（Codex 侧另含 `apply_patch`） | 拦截危险 Bash、受保护路径写入、push 到 `main`/`master`；另薄接线 doc-lifecycle 完整性拦截（判定本体在 `scripts/check-doc-lifecycle.py`，见下） | 硬阻止（exit 2） |
...
| `agent_name_set.py` | —（agent 调用的 setter） | agent 选定名后调用：写 `.agent-identity` + 默认 `paseo rename`（`AGENT_NO_AUTORENAME=1` 关）+ upsert `memory/agents-roster.md`；`--register --paseo-id <id>` 只登记子 agent（`spawn` skill launcher 用，不改自身） | N/A |
...
对 brief/plan/review/decision 四类文档与 `memory/doc-lifecycle.yaml` 的 `Edit|Write|apply_patch`
写入，加载 `scripts/check-doc-lifecycle.py` 的 `pretooluse_reason()`（与 validator 同一份逻辑）：
...
- 写注册表引入悬空引用/非法枚举/缺 approval 证据 → 拦。
- **边界**：只判可判定事实，不判研究方向/风险/是否真收敛（那是 human gate）；提示明确指出缺哪个字段/哪条引用悬空。
- 逃生：`DOC_LIFECYCLE_SKIP=1`（human 明示时用；validator 仍事后校验）。判定层异常一律保守放行，不反噬安全地板。
- 状态语义/注册表 schema 见 `plans/ANATOMY.md`。
