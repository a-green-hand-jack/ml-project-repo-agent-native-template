...
| 文件 | 事件 | 作用 | 阻止能力 |
| --- | --- | --- | --- |
| `pre_tool_guard.py` | `PreToolUse(Bash\|Edit\|Write)` | 拦截危险 Bash、受保护路径写入、push 到 `main`/`master`；另薄接线 `scripts/check-agent-conflicts.py` 拦多 agent ownership 冲突/写错 worktree（`AGENT_CONFLICT_SKIP=1` 绕过，见 `.agent/multi-agent-control-plane.md`） | 硬阻止（exit 2） |
