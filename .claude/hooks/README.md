...
- 写注册表引入悬空引用/非法枚举/缺 approval 证据/kind 与路径类别不符（谎报 kind）→ 拦。
- 删除/移走注册表（`apply_patch` Delete/Move、Bash `rm`/`mv`/`git rm` 等）→ 拦：注册表是治理面，
  删除等于静默关闭校验（validator 侧「有受管文档但注册表缺失」同为 error）。
- `apply_patch` 的 Update 会按 hunk 重建 patch 后全文再判定（与 Edit 同语义）；无法可靠重建时
  **保守拦截**并提示改用 Edit——不静默放行状态跃迁。
