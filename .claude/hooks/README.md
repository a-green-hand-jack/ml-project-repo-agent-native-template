...
`pretooluse_reason()`（与 validator 共用 parser 和局部判定；跨文件规则只在 validator）：
...
- 文档状态锚点与注册表状态是跨文件不变量：状态流转允许两次工具调用之间短暂不一致，hook 不在
  每次 Edit/Write 后要求两处立即相等；`check-doc-lifecycle.py` 在 commit/治理门禁粒度必须拒绝最终不一致。
