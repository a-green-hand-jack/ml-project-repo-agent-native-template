...
只在 source ∈ {compact, clear} 时回注（startup/resume 是新开/续接，不需要）。
SessionStart 的 stdout（exit 0）会并入新上下文——这是回注通道。
为避免回注本身又吃掉大量上下文，正文截断到上限。永远 exit 0。
...
def main() -> None:
    ...
    if not STATUS_FILE.is_file():
        ...
        print(
            f"[continuity] {source} 后未找到 {STATUS_FILE.name}。"
    ...
    print(
        f"[continuity] {source} 后回注 memory/current-status.md（{fresh}）。"
