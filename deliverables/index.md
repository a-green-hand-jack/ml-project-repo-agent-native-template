...
- **路径**：交付物在 repo 内的**具体 regular file**；必须 repo-relative，拒绝 absolute、
  `..`、resolve 越界、symlink escape 与目录。Markdown 可嵌 marker；其他格式走人工 review。
...
- `evidence=` 可选，逗号分隔；若填写，每个 id 必须存在于 `lab/research/evidence.yaml`，
  该 evidence 的 `supports_claim` 必须等于 marker 的 claim id，且 evidence id 已列入
  该 claim 的 `evidence` 列表；单向孤边不得出现在 marker 中。
...
- 校验：`python scripts/check-provenance-chain.py`（由 `validate-governance.py` 拉起）。
  ...
  都会被拦截。
- **「齐全=是」还必须有机器可见的支撑动作（二选一）**：交付物 `.md` 正文的 claim
  marker 覆盖本表该行列出的**全部** claim，或本表该行内引用
  `human/reviews/results/` 下**存在且路径安全的 regular file** review 记录
