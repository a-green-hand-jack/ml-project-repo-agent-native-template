...
2. 为每个 asset 记录关键字段（共同最小 schema 见 `.agent/artifact-policy.md`「provenance 链」，全部 7 类 index 统一，以 `result-index.yaml` 为基准）：`id` / 描述性字段（`summary`，图表/模型对应 `caption`/`name`）/ `location`(统一路径/URI 字段) / `how_to_inspect`(如何检视) / `commit` + `config` + `run_id`(可复现三元组，`run_id` 须指向 experiment-ledger 中已闭环 run；trace/dataset 特例：无 run 来源时可留占位，填了才校验) / `supports`(支持哪个 claim，图表里是 `supports_claim`；model 另有 `checkpoint_ref` 指向 checkpoint-index) / `status` / checksum 三件套（`checksum` + `checksum_algorithm: sha256`，或无法校验时 `checksum_unavailable_reason` 固定枚举 + `checksum_unavailable_justification` 非占位理由；多文件产物可用 `manifest`）/ `missing_metadata` / `archive_recommendation`(checkpoint 里是 `archive_after`)。index 文件顶层带 `schema_version`。
