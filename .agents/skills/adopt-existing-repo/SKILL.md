...
> Codex adapter: generated from `.claude/skills/adopt-existing-repo/SKILL.md`. Do not edit this copy by hand; run `python scripts/sync-codex-adapters.py`.
...
- 输入：已有 Git repo 路径、每个 phase 都必传的 `--origin <owner/repo>`（不从 remote 或路径猜测）。
...
2. 运行 discover，看归类计划（不落盘时用 `--dry-run`）：
   ...
   ```bash
   python scripts/adopt-existing-repo.py /path/to/repo --phase discover --project-name <slug> --origin <owner/repo>
   python scripts/adopt-existing-repo.py /path/to/repo --phase discover --dry-run --project-name <slug> --origin <owner/repo>
   ```
...
3. 运行 baseline，记录 tracked file hash、root entries、原生测试结果：
   ...
   python scripts/adopt-existing-repo.py /path/to/repo --phase baseline \
     --test-command "python -m unittest discover" --origin <owner/repo>
   ```
...
5. 运行 normalize，按 discover 的归类计划把 `conservative_import` 项移动到
   ...
   `conflict`，停下并报告（不静默继续）。只有无 blocker 的成功 normalize 在所有 move 后原子
   create/confirm `.template.toml`；dry-run、scaffold-only 和 `--allow-blocked-normalize` 不写。
   同 origin 锚点保留现有 version；异源、malformed、symlink 锚点均在 move 前 fail-closed。新锚点
   是 root classifier 的受治理 control item，第二次 adoption 不会把它当 root pollution。
...
7. 若只想无人值守跑完整保守路径：
   ...
   ```bash
   python scripts/adopt-existing-repo.py /path/to/repo --phase all --policy conservative --origin <owner/repo>
...
- Codex trust 是 out-of-band human gate：在目标 repo 启动 Codex，运行 `/hooks` 审阅并信任；
  fresh session 后用 `python scripts/check-codex-hook-runtime.py --status` 验 receipt。adoption 不改
  用户全局 trust 状态，也不把缺 receipt 的状态报成已受保护；撤销走 `/hooks`。
