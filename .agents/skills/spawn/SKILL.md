...
> Codex adapter: generated from `.claude/skills/spawn/SKILL.md`. Do not edit this copy by hand; run `python scripts/sync-codex-adapters.py`.
...
### 位置选择约定（issue #47，human 拍板）

开新 agent 时「开在哪一层」按 worktree 与否钉死——位置服务于 human 手动切 tab/归档/监督的可操作性，
不是 agent 图省事：

- **没有建新 git worktree** → 只在**当前 Workspace 里开新 Agent tab**；不新建 Workspace、不新建 Project。
- **建了新 worktree** → 可以开**新 Workspace + 新 tab**（Paseo 会为 worktree 关联 Workspace）。
- **永远不要开新 Project**（不 add/open 新的目录根进 Paseo sidebar）。

违反的典型症状：agent 散落在多个 Project 里，human 无法一屏监督。launch 前自检这三条。
