...
related_files:
  ...
  - ../scripts/check-codex-hook-runtime.py
maintenance: |
...
| 路径 | 角色 |
| --- | --- |
| `config.toml` | Codex project config：subagent 并发/深度限制与 lifecycle hooks；每条 hook 定义带共享 bundle SHA，脚本变化后必须刷新并经 `/hooks` 重审。 |
...
- `scripts/check-codex-hook-runtime.py` 审计 hook bundle；`SessionStart` 调
  `.claude/hooks/codex_runtime_receipt.py` 写 gitignored receipt，只有当前 bundle 的 fresh receipt 才能证明已加载。
...
- Codex project config 只在用户信任本 repo 的 `.codex/` layer 后加载；稳定入口是 Codex CLI `/hooks`，
  bootstrap/adoption 不修改 `~/.codex/config.toml` 内部 trust 状态。
